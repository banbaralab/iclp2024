...
def is_dominating_set(vnum, edges, state):
    """
    ...
    ----------
    bool
      True if ``state`` is a dominating set, False otherwise.
    """
    X = [False] * (vnum + 1)
    for s in state:
        X[s] = True
...
def main():
    ...
    if not is_exist:
        ...
        return
    # Check that answer is exist
    if not answers:
        print('[Code03] (Answer: UNKNOWN) The answer is not found: reachability unknown')
        return;
        
    # Check that the initial state match the start state
    if s != answers[0]:
        print('[Code10] The initial state must be equal to the start state')
        raise ValidationError(
            '[Code10] The initial state must be equal to the start state')

    # Check that the last state match the target state
    if t != answers[-1]:
        print('[Code11] The last state must be equal to the target state')
        raise ValidationError(
            '[Code11] The last state must be equal to the target state')

    # Check that each state is an independent set.
    for i, a in enumerate(answers):
        if not is_dominating_set(vnum, edges, a):
            print('[Code12] The ' + ordinal(i + 1) +
                ' state is not a dominating set')
            raise ValidationError(
    ...
    for i in range(len(answers) - 1):
        ...
        if len(diff) != 1:
            print('[Code13] Each dominarting set in the sequence results from '
                'the previous one by moving exactly one token to another node')
            raise ValidationError(
                '[Code13] Each dominating set in the sequence results from '
                'the previous one by moving exactly one token to another node')
